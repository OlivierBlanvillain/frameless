...
class CreateTests extends TypedDatasetSuite with Matchers {
  ...
  test("creation using X4 derived DataFrames") {
    // def prop[
    // A: TypedEncoder,
    // B: TypedEncoder](data: List[X2[A, B]]): Prop = {
    //   val ds = TypedDataset.create(data)
    //   TypedDataset.createUnsafe[X2[A, B]](ds.toDF()).collect().run() ?= data
    // }
    ...
    val ite = new RecordEncoder[X1[Int], R, R]

    val ee = ExpressionEncoder[X1[Int]]

    val te = new TypedEncoder[X1[Int]]()(ee.clsTag) {
      def nullable: Boolean = ite.nullable
      def jvmRepr = ite.jvmRepr
      def catalystRepr = ite.catalystRepr
      def fromCatalyst(path: Expression): Expression = ite.fromCatalyst(path)
      def toCatalyst(path: Expression): Expression = ite.toCatalyst(path)

      override val lol = true
      override val lel = ee
      override val TT = X1(99)
    }


    val x = TypedDataset.createUnsafe[X1[Int]](TypedDataset.create(data)(te, implicitly).toDF())(te).collect().run()
    // x ?= data
